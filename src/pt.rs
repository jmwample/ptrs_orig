...
fn get_proxy_url() -> Result<Url, Box<dyn std::error::Error>> {
	let tor_pt_proxy = match env::var(TOR_PT_PROXY) {
		Ok(url) => url,
		Err(err) => match err {
			env::VarError::NotPresent => return Err(Box::new(PTError::NoProxyRequested)),
	...
	if url.scheme() == "" {
		return Err(Box::new(PTError::ProxyError(String::from("missing scheme"))))
	}

	if !url.has_authority() {
		return Err(Box::new(PTError::ProxyError(String::from("missing authority"))))
	}
...
mod tests {
	...
	fn proxy_url_parse() {
		...
		match get_proxy_url() {
			Ok(url) => {
				panic!("empty environment returned {}", url);
			},
			Err(err) => {
				assert_eq!(err.to_string(), PTError::NoProxyRequested.to_string(), "empty environment returned unexpected error: {}", err);
		...
		for (input, expected) in good_cases {
			...
			match get_proxy_url() {
				Ok(u) => {
					...
					assert_eq!(&url, expected, "TOR_PT_PROXY={} → {} (expected {})", input, &url, expected);
				}
				Err(err) => {
					panic!("TOR_PT_PROXY={} unexpectedly returned an error: {}", input, err);
				}
			}
		}
		...
		for input in bad_cases {
			...
			match get_proxy_url() {
				Ok(url) => {
					panic!("TOR_PT_PROXY={} unexpectedly succeeded and returned {:?}", input, url);
				},
				Err(_) => {},
			}
		}
