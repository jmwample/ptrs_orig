...
fn default_server_config_with_ca(ca_cert: certs::CertSet) -> Result<Arc<rustls::ServerConfig>> {

	let (certs, ky) = gen_self_signed_cert()?;

	let mut cert_store = rustls::RootCertStore::empty();

	cert_store.add(ca_certs.ca_pem);
...
fn gen_self_signed_cert() -> Result<(Vec<rustls::Certificate>, rustls::PrivateKey)> {
	...
	let certificate: Vec<rustls::Certificate> = certs.into_iter().map(rustls::Certificate).collect();
...
mod test {
	...
	async fn async_tls_rustls_read_write() -> Result<()> {
		...
		tokio::spawn(async move {
			let server = Server {
				config: Config {
					client_cfg: None,
					server_cfg: Some(default_server_config().with_ca(cert_set.ca_pem).unwrap()),
				}
			};
	...
	#[test]
	fn rcgen_cert_generation() -> std::io::Result<()> {
	    // Generate a certificate that's valid for "localhost" and "hello.world.example"
	    let subject_alt_names = vec!["hello.world.example".to_string(), "localhost".to_string()];
	    ...
	    // Get rustls compatible cert
	    let mut cert_r = Cursor::new(cert.serialize_pem().unwrap());
	    let certs = rustls_pemfile::certs(&mut cert_r)?;
	    let certificates = certs.into_iter().map(rustls::Certificate).collect();

	    // Get rust compatible private key
	    let mut privkey_r = Cursor::new(cert.serialize_private_key_pem());
	    let mut keys = rustls_pemfile::pkcs8_private_keys(&mut privkey_r)?;
	    let key = match keys.len() {
			0 => Err(format!("No PKCS8-encoded private key found")).unwrap(),
			1 => rustls::PrivateKey(keys.remove(0)),
			_ => Err(format!("More than one PKCS8-encoded private key found")).unwrap(),
	    };
